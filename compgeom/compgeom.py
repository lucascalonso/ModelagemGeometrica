...
class CompGeom:
    ...
    def getClosestPointSegment(_p1, _p2, _p):
        # Robust implementation using coordinates to avoid undefined variables
        # Returns: distance, closest_point (Pnt2D), t (param in [0,1])
        from compgeom.pnt2d import Pnt2D

        # Vector from p1 to p2 and p1 to p
        v12x = _p2.getX() - _p1.getX()
        v12y = _p2.getY() - _p1.getY()
        v1px = _p.getX() - _p1.getX()
        v1py = _p.getY() - _p1.getY()

        denom = v12x * v12x + v12y * v12y

        if denom == 0.0:
            # Degenerate segment: both endpoints coincide
            t = 0.0
            pC = Pnt2D(_p1.getX(), _p1.getY())
        else:
            t = (v12x * v1px + v12y * v1py) / denom
            if t <= 0.0:
                t = 0.0
                pC = Pnt2D(_p1.getX(), _p1.getY())
            elif t >= 1.0:
                t = 1.0
                pC = Pnt2D(_p2.getX(), _p2.getY())
            else:
                pC = Pnt2D(_p1.getX() + v12x * t, _p1.getY() + v12y * t)
